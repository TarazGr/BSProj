...
if not os.path.exists(TMP_DIR + '/closed_train_set.pkl') or not os.path.exists(TMP_DIR + '/closed_test_set.pkl'):
    print('Loading dataset for closed-set train...')
    dataset = []
    category2index = {}
    index = 0
    ...
    closed_train_set, closed_test_set = dp.closed_set_split(dataset, category2index, train_size=0.75)
    del dataset
    pickle.dump(closed_train_set, open(TMP_DIR + '/closed_train_set.pkl', 'wb'))
...
with tf.Session(graph=graph) as session:
    ...
    try:
        saver.restore(session, os.path.join(TMP_DIR, 'closed_model.ckpt'))
    ...
    for step in tqdm.tqdm(range(ITERACTIONS), desc='Training Siamese Network'):
        batch, label = dp.get_batch(closed_train_set, BATCH_SIZE)
